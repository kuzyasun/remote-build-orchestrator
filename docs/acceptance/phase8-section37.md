# §37 Acceptance checklist (Phase 8)
...
|---|---|---|---|
| 1 | AI clients verified config + MCP smoke matrix | not_verified | `docs/compatibility/matrix.json` (product clients); harness `pass` via `phase8-smoke.test.ts` |
| 2 | stdio & Streamable HTTP same schemas/results | pass | `apps/controller/test/transports.test.ts`, `phase8-smoke.test.ts` |
...
| 11 | QEMU script minutes + logs | environment_gated | `phase7-workloads.test.ts` fake QEMU; real QEMU gated |
...
| 14 | Disconnect keeps logs; no duplicate attempt | pass | `phase6-reliability.test.ts` |
...
| 23 | Benchmark report metrics | pass | `phase7-benchmark.test.ts` + `docs/ops/observability-report.md` |

## Sign-off

Phase 8 treats product AI-client cells as `not_verified` until real client evidence exists (Option A).
